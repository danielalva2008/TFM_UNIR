...
results
with open('PartidoMoradoS1OK.json', 'w') as file:
...
twet_data= open('PartidoMoradoS1OK.json', 'r').read()
...
with open('PartidoMoradoS1OK.json') as train_file:
...
total_partido_morado = df.groupby(['POS1']).agg(
                                  {'numero': 'sum'
                                  }).reset_index()
...
df_analisis_sentimiento_partido_morado = total_partido_morado.T
df_analisis_sentimiento_partido_morado.reset_index(drop=True,inplace=True)
df_analisis_sentimiento_partido_morado = df_analisis_sentimiento_partido_morado.rename(columns={0:df_analisis_sentimiento_partido_morado.loc[0, 0]})
df_analisis_sentimiento_partido_morado = df_analisis_sentimiento_partido_morado.rename(columns={1:df_analisis_sentimiento_partido_morado.loc[0, 1]})
df_analisis_sentimiento_partido_morado = df_analisis_sentimiento_partido_morado.rename(columns={2:df_analisis_sentimiento_partido_morado.loc[0, 2]})
df_analisis_sentimiento_partido_morado['cant_tot_extraidos']=len(df)
df_analisis_sentimiento_partido_morado.drop([0],axis=0,inplace=True)
#df_analisis_sentimiento_partido_morado
...
df_analisis_tendencia_partido_morado = pd.DataFrame(data=xd,columns=['termino','frecuencia'])
...
#df_analisis_tendencia_partido_morado


# In[18]:


df_t_usuarios_partido_morado = df.groupby(['created_at','POS']).agg(
                                  {'numero': 'sum'
                                  }).reset_index()

df_t_usuarios_partido_morado.rename(columns={'created_at': 'fecha', 
                                    'POS': 'sentimiento','numero':'tweet'}, inplace=True)
#df_t_usuarios_partido_morado
...
#creamos la tabla analisis_sentimiento_partido_morado
analisis_sentimiento_partido_morado = db.analisis_sentimiento_partido_morado 
records = json.loads(df_analisis_sentimiento_partido_morado.T.to_json()).values()
db.analisis_sentimiento_partido_morado.insert_many(records)


# In[21]:


df_t_partido_morado =pd.DataFrame()
df_t_partido_morado['id_str']         =df['id_str']
df_t_partido_morado['screen_name']    =df['screen_name']
df_t_partido_morado['created_at']     =df['created_at']
df_t_partido_morado['full_text']      =df['full_text']
df_t_partido_morado['favorite_count'] =df['favorite_count']
df_t_partido_morado['retweet_count']  =df['retweet_count']
df_t_partido_morado['location']       =df['location']
df_t_partido_morado['source']         =df['source']
df_t_partido_morado['pos']            =df['POS']


# In[22]:


df_usuario_partido_morado = db.t_usuario_partido_morado
records = json.loads(df_t_partido_morado.T.to_json()).values()
db.t_partido_morado.insert_many(records)


# In[23]:


analisis_tendencia_partido_morado = db.analisis_tendencia_partido_morado
records = json.loads(df_analisis_tendencia_partido_morado.T.to_json()).values()
db.analisis_tendencia_partido_morado.insert_many(records)


# In[24]:


t_usuarios_partido_morado = db.t_usuarios_partido_morado
records = json.loads(df_t_usuarios_partido_morado.T.to_json()).values()
db.t_usuarios_partido_morado.insert_many(records)
