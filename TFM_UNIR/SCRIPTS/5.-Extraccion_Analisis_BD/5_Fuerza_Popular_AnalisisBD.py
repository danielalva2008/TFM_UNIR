...
# **Analisis del Partido Fuerza Popular**
...
#search_words="fujimori"
search_words="#FuerzaPopular"
api = tweepy.API(auth)                                                                                                 
...
results
with open('FuerzaPopularS1OK.json', 'w') as file:
...
twet_data= open('FuerzaPopularS1OK.json', 'r').read()
...
with open('FuerzaPopularS1OK.json') as train_file:
...
total_fuerza_popular = df.groupby(['POS1']).agg(
                                  {'numero': 'sum'
                                  }).reset_index()
...
df_analisis_sentimiento_fuerza_popular = total_fuerza_popular.T
df_analisis_sentimiento_fuerza_popular.reset_index(drop=True,inplace=True)
df_analisis_sentimiento_fuerza_popular = df_analisis_sentimiento_fuerza_popular.rename(columns={0:df_analisis_sentimiento_fuerza_popular.loc[0, 0]})
df_analisis_sentimiento_fuerza_popular = df_analisis_sentimiento_fuerza_popular.rename(columns={1:df_analisis_sentimiento_fuerza_popular.loc[0, 1]})
df_analisis_sentimiento_fuerza_popular = df_analisis_sentimiento_fuerza_popular.rename(columns={2:df_analisis_sentimiento_fuerza_popular.loc[0, 2]})
df_analisis_sentimiento_fuerza_popular['cant_tot_extraidos']=len(df)
df_analisis_sentimiento_fuerza_popular.drop([0],axis=0,inplace=True)
#df_analisis_sentimiento_fuerza_popular
...
df_analisis_tendencia_fuerza_popular = pd.DataFrame(data=xd,columns=['termino','frecuencia'])
...
#df_analisis_tendencia_fuerza_popular


# In[18]:


df_t_usuarios_fuerza_popular = df.groupby(['created_at','POS']).agg(
                                  {'numero': 'sum'
                                  }).reset_index()

df_t_usuarios_fuerza_popular.rename(columns={'created_at': 'fecha', 
                                    'POS': 'sentimiento','numero':'tweet'}, inplace=True)
#df_t_usuarios_fuerza_popular
...
#creamos la tabla analisis_sentimiento_fuerza_popular
analisis_sentimiento_fuerza_popular = db.analisis_sentimiento_fuerza_popular 
records = json.loads(df_analisis_sentimiento_fuerza_popular.T.to_json()).values()
db.analisis_sentimiento_fuerza_popular.insert_many(records)


# In[21]:


df_t_fuerza_popular =pd.DataFrame()
df_t_fuerza_popular['id_str']         =df['id_str']
df_t_fuerza_popular['screen_name']    =df['screen_name']
df_t_fuerza_popular['created_at']     =df['created_at']
df_t_fuerza_popular['full_text']      =df['full_text']
df_t_fuerza_popular['favorite_count'] =df['favorite_count']
df_t_fuerza_popular['retweet_count']  =df['retweet_count']
df_t_fuerza_popular['location']       =df['location']
df_t_fuerza_popular['source']         =df['source']
df_t_fuerza_popular['pos']            =df['POS']


# In[22]:


df_usuario_fuerza_popular = db.t_usuario_fuerza_popular
records = json.loads(df_t_fuerza_popular.T.to_json()).values()
db.t_fuerza_popular.insert_many(records)


# In[23]:


analisis_tendencia_fuerza_popular = db.analisis_tendencia_fuerza_popular
records = json.loads(df_analisis_tendencia_fuerza_popular.T.to_json()).values()
db.analisis_tendencia_fuerza_popular.insert_many(records)


# In[24]:


t_usuarios_fuerza_popular = db.t_usuarios_fuerza_popular
records = json.loads(df_t_usuarios_fuerza_popular.T.to_json()).values()
db.t_usuarios_fuerza_popular.insert_many(records)
